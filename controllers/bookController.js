...
exports.book_list_public = function (req, res, next) {
  Book.find({}, "title author")
    .sort({ title: 1 })
    .populate("author")
    .exec(function (err, list_books) {
      ...
      //Successful, so render
      res.render("book_list_public", { title: "Book List", book_list: list_books });
    });
};
...
exports.book_detail_public = (req, res, next) => {
  async.parallel(
    {
      book(callback) {
        Book.findById(req.params.id)
